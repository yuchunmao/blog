...
const BlogBg = () => {
  if (!document.getElementById('db_BlogBg')) {
...
const CanvasNest = () => {
  // 创建 dom
  if (!document.getElementById('db_bg_canvas_nest')) {
    const canvasElm = document.createElement('canvas');
    canvasElm.id = 'db_bg_canvas_nest';
  ...
  if (!document.getElementById('canvas_nest_js')) {
    ...
    jsElm.src = '/blog/script/canvas-nest.js';
...
const GrainParallax = () => {
  // 插入 css
  if (!document.getElementById('db_grain_parallax_css')) {
    const cssElm = document.createElement('link');
    cssElm.type = 'text/css';
    cssElm.rel = 'stylesheet';
    cssElm.href = '/blog/script/grain-parallax.css';
    cssElm.id = 'grain_parallax_css';
    document.body.appendChild(cssElm);
  }

  // 创建 dom
  if (!document.getElementById('db_bg_grain_parallax')) {
    const boxElm = document.createElement('div');
    boxElm.id = 'db_bg_grain_parallax';
...
<div id="particles-foreground"></div>
  ...
  // 插入 js
  if (!document.getElementById('db_grain_parallax_js')) {
    var jsElm = document.createElement('script');
    jsElm.id = 'db_grain_parallax_js';
    jsElm.type = 'text/javascript';
    jsElm.src = '/blog/script/grain-parallax.js';
    document.body.appendChild(jsElm);
  }
};
