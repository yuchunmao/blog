...
const CheckScrollTopClass = () => {
  ...
  if (blogHeroElm) {
    themeElm.classList.add('db-blog-hero');
  } else {
    themeElm.classList.remove('db-blog-hero');
  }

  const scrollTop = document.documentElement.scrollTop;
  if (scrollTop < 60) {
    themeElm.classList.add('db-scroll-top');
  } else {
    themeElm.classList.remove('db-scroll-top');
  }

  if (blogHeroElm) {
    if (scrollTop < blogHeroElm.clientHeight - 30) {
      themeElm.classList.add('db-scroll-blog-hero-inner');
    } else {
      themeElm.classList.remove('db-scroll-blog-hero-inner');
    }
  }
};
...
// 当导航栏在首页导到达顶部时
.theme-container.db-scroll-top.db-blog-hero {
  .vp-navbar {
    box-shadow: none;
  }
}
...
.theme-container.db-blog-hero.db-scroll-top {
  .vp-navbar {
...
[data-theme='light'] {
  ...
  .theme-container.db-scroll-blog-hero-inner {
    // 搜索按钮
    .search-pro-button {
      color: #eee;
      &:hover {
